...
1) Телескопические конструкторы для счетов были удалены. Проблема решена с использованием паттерна Builder (Строитель).
2) Внутри классов счетов добавлены классы AccountBuilder, CreditAccountBuilder, DebitAccountBuilder,
   DepositAccountBuilder, которые заменили конструкторы счетов.